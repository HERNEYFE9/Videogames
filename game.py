...
def solicitar_numero_jugadores():
    while True:
        try:
            num_jugadores = int(input("Ingresa la cantidad de jugadores (2-4): "))
...
def jugar():
    ...
    while not juego_terminado:
        for i in range(num_jugadores):
            ...
            print(f"Jugador {i+1} ha lanzado: {dado1} y {dado2}")
            ...
            avance = dado1 + dado2
            posiciones[i] += avance
            print(f"Jugador {i+1} avanza {avance} posiciones. Ahora está en la posición {posiciones[i]}.")
